...
## 0.27.11
- `drtools/panels`: new `create_chart_panel` tool (ported from wren-mcp) — runs LLM-authored Python charting code in the isolated DataRobot workload sandbox (`execute_code`) over a source panel and persists the result as a Chart panel. Registered under the `dr_panels` MCP tool category. Sources: a Dataset panel bound as a polars DataFrame `df`, or a Json panel bound as `data` (folium's GeoJSON input). Three charting libraries, each with its own validated stored-blob shape under the BPA frontend's `ChartPayload` union — `plotly` → `{"format": "plotly", "spec": <figure-json>}`, `altair` → `{"format": "altair", "spec": <vega-lite-spec>}`, `folium` → `{"format": "html", "html": "<html>…"}` (note folium's discriminator is `html`, not `folium`) — all with `content_type="application/json"`. plotly and altair need no package in the sandbox image since both formats are plain JSON; folium does, and a sandbox image missing it produces an error naming `DR_MCP_SANDBOX_IMAGE` rather than blaming the agent's code.
