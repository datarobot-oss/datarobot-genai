...
## 0.29.51
- `dragent`: add `a2a.enable_skills_in_redacted_card` opt-in. Off by default (redacted agent cards served to unauthenticated callers keep `skills` empty, as before); when set to `true` alongside `enable_unauthenticated_well_known_route`, the redacted card's `skills` match the authenticated card's.
