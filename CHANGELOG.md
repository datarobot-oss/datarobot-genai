...
- **How a server is addressed no longer decides how it is authenticated.** `<name>_mcp_auth_provider` names an `authentication:` entry — the same string a `workflow.yaml` function group would use — and can equally be set in `config.py` or the YAML. Leaving it unset derives a default from the address kind that reproduces the previous behaviour exactly, so a configuration that never sets it is unaffected. What is newly possible: a DataRobot-hosted MCP server addressed by `url` can carry a DataRobot token, which is what makes the platform's global MCP reachable at all — previously `url` meant third-party and third-party meant no credentials, in the same `if` branch that built the URL. A DataRobot-hosted server can equally be reached anonymously. `x-datarobot-api-key` is now `<name>_mcp_api_key_header`, separate from the identity because it is a property of the workload route.
- **A `url` server on a host other than the DataRobot endpoint cannot receive DataRobot credentials** unless `<name>_mcp_trust_host=true` is set. This replaces the previous guarantee — that a third-party server simply could not be given a token — which making credentials declarable necessarily relaxes.
