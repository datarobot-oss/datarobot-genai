...
- `dragent`: deployed-agent chats now render their full trace in the Agentic Playground. The playground walks down from buzok's `chat_completion_deployment` bridge span (under `experiment_container-<use_case_id>`), but the outbound hop to the deployment (aiohttp client / prediction router) re-injects its own span id as the `traceparent` parent and never exports it there, so the agent subtree was an unreachable disconnected root. A new `PlaygroundTraceLinkMiddleware` materializes that missing parent - one span whose id equals the incoming `traceparent` parent, tagged `experiment_container-<use_case_id>` - so the agent subtree becomes reachable. No-op unless `DATAROBOT_USE_CASE_ID` is set (the app passes it as a runtime parameter).
