...
## 0.15.99
- `drtools/sandbox`: added a `Sandbox` protocol with two container-backed implementations sharing one runner wire-contract (`sandbox/protocol.py`): `LocalDockerSandbox` (dev/test — runs the sandbox image in a local Docker container via the `docker` CLI) and `DataRobotWorkloadSandbox` (production — submits a short-lived workload to the DataRobot workload-api). Both run the same DRUM-built image with the same `__DR_SANDBOX_RESULT__:` marker protocol. `execute_code` is exposed as a plain async function (not an MCP tool yet — kept separate so it doesn't override FastMCP CodeMode's `execute` tool; a later PR adds the `MCP_SANDBOX`-gated tool layer that calls it). Workload security context (read-only rootfs, drop ALL caps, RuntimeDefault seccomp, no privilege escalation) is gated by `ENABLE_WORKLOAD_API_SECURITY_CONTEXT`, evaluated per-user via the thread-safe `request_user_dr_client()` + `FeatureFlag` (`drtools.core`); fails safe to cluster defaults on FF lookup error.
