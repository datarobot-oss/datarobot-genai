...
## 0.25.2
- `application-utils`: **Chat-history models & repositories** — new `datarobot_genai.application_utils.chat_history` sub-package (models + persistence layer) built on the persistence ORM. Adds `Chat`/`Message` domain models with typed nested `ToolCall`/`Reasoning`, the `Role`/`MessageStatus` enums, the `*Create`/`*Update`/`MessagePublic` DTOs, and `ChatRepository`/`MessageRepository` plus the structural `ChatRepositoryLike`/`MessageRepositoryLike` protocols and `ChatSessionRegistry` (idempotent create via dedup-key adopt, indexed `thread_id` lookup, bounded RMW retry on the `createdAt` concurrency token). Every cold `uuid → location` lookup (chat→session, message→chat, tool-call/reasoning→parent message) resolves through a dedup-keyed `EntityLocator` secondary index — an indexed O(1) point lookup sharing the chats' Memory Space (isolated by the `//loc/` description prefix) — instead of a full-space session scan; the in-process resolution caches are bounded LRUs. Locator writes are best-effort (the event/session remains the source of truth), so this backend offers no strong cross-replica read-after-write consistency for uuid-addressed lookups. Models are transport-agnostic — this layer imports no `ag_ui`. Public import: `from datarobot_genai.application_utils.chat_history import Chat, Message, EntityLocator, ChatRepository, MessageRepository`.
