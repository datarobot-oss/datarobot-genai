...
## 0.29.15
- `drtools/otel` (new package): seven read-only OpenTelemetry observability tools — `otel_traces_list`, `otel_trace_get`, `otel_span_payload_get`, `otel_logs_list`, `otel_metrics_catalog_list`, `otel_metrics_values_get`, `otel_entity_stats_get` — for inspecting a DataRobot entity's traces, logs, and metrics from an agent. Gated behind a new `MCPToolConfig.enable_otel_tools` field (env `ENABLE_OTEL_TOOLS`, default `false`) and registered under a new standalone `dr_otel` tool category (label "Observability"), alongside `dr_documentation`/`dr_use_cases`/`dr_deployments` rather than nested under an existing parent. A raw trace is unusable passed through as-is: measured p50/p95/max is 1,129 / 809,639 / 1,560,241 tokens, and span count does not predict size (an 86x tokens-per-span spread between two measured traces), so every tool in the package projects its response rather than forwarding the upstream payload. `otel_trace_get`'s default `view="summary"` is the load-bearing case — a flat ~133 tokens/span span-tree projection (4,936 tokens on the 1.56M-token worst-measured trace, 0.3%) with per-span `payload_chars`/`payload_fields` accounting, so an agent sees where the payload mass is before drilling into one span's full text with `otel_span_payload_get`. All truncation (`drtools/otel/truncation.py`) is a character budget, not a token count — `drtools` may import only `drtools`/`drmcputils`, so no tokenizer dependency (no `tiktoken`) was added; a measured 3.1 chars/token on this JSON shape sets the conversion. Duplicate and derived semconv fields (Traceloop/NAT/OpenInference all independently write the same prompt/completion text — 64.5% of the worst trace was exact duplication) are deduplicated by `canonical_attributes()` and every drop is named in the response (`dropped_as_duplicate`/`dropped_semconv`), never silent; a 403 from any trace/log/metrics tool is documented in its own description as most likely `GENAI_EXPERIMENTATION`/`AGENTIC_PREDICTIVE_GOVERNANCE_BUILDER` configuration rather than missing data, while `otel_entity_stats_get` needs neither and can be used as a preflight check on a cluster where the rest 403. New `OtelQueryApiClient` (`drtools/core/clients/datarobot_otel_query.py`) and `require_object_id`/`require_trace_id` validators (`drtools/core/utils.py`) support the tool layer; no write or bulk endpoints are exposed.
- `drtools/otel` hardening from pre-merge review: `min_trace_cost`/`max_trace_cost` accept fractional amounts (`float`, was `int`); `otel_span_payload_get` pages through the trace's spans instead of stopping at the first 100, matches trace/span ids case-insensitively (`require_trace_id` now lowercases), and requires `fields` alongside a non-zero `field_offset` so a continuation offset cannot blank every short field (`status_message` may be named in `fields` to continue it); non-string attribute values (OTLP arrays, JSON-decoded message lists) are counted, deduplicated and windowed over their serialized JSON (`"serialized": true` in the truncation record) instead of bypassing every cap; `_normalize_keys` no longer mangles user-defined guard names under `metrics.prompt_guards`/`response_guards`; `MCP_CLI_CONFIGS=otel` now enables the package (`MCP_CLI_OPTS` gained its `otel` row).
- docs: new `docs/src/drtools/otel.md` — the tool set, the summary→drill-down flow, the size numbers above, the 403 causes, and a section on pointing a coding harness's `.mcp.json` at a local `drmcp` server (previously undocumented outside `.taskfiles/drmcp.yml`). `otel_trace_get`'s `span_limit`/`max_field_chars`/`max_total_chars` defaults are confirmed there against a real agent deployment (130 KB - 1.67 MB raw traces): summary output stays flat near 14,000 characters across a 13x growth in raw trace size, and only `max_total_chars` binds, at ~94% utilization.
