...
- `dragent`: `a2a.mount_path` is now validated instead of accepted verbatim. Each slash-separated segment must be RFC 3986 unreserved characters (letters, digits, `- . _ ~`) and must not begin with a dot, so values that used to fail silently are rejected at config load: Starlette path-parameter syntax such as `{id}` (which compiled to a wildcard swallowing unrelated paths) or `{path}` (which crashed at startup), values carrying URI syntax such as `?`, `#` or `%` (which made the advertised agent card URL resolve somewhere other than the mount, leaving A2A undiscoverable), empty interior segments, and the RFC 8615 reserved `.well-known` namespace.
- `dragent`: mounting A2A on a path another route already serves is now rejected at startup, naming the conflicting route; a mount path that merely overlaps another route's prefix logs a warning. An exact collision suppresses the redirect that normally makes the slashless `POST /{mount_path}` reach the JSON-RPC endpoint, so an A2A client omitting the trailing slash would otherwise silently receive the other route's response.
