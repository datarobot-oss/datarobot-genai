...
- `drmcp`: `dr_mcp_tool` / `dr_core_mcp_tool` bypass FastMCP-native auth checks passed on `auth=` (`fastmcp.server.auth.require_scopes`, `restrict_tag`, custom checks) while `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` is off, with a warning naming the tool. FastMCP evaluates those at the tool level against `ctx.token`, which it reads from the `request.scope["user"]` our token handler sets only while the gate is on — so with the gate off no token reaches FastMCP and such a check would hide the tool from every caller. With the gate on they are forwarded unchanged and run against the parsed token (hiding the tool rather than answering 403, and invisible to `scopes_supported` and the REST `required_scopes` field) — declare scopes with `datarobot_genai.drmcp.require_scopes` instead; our own declarations are never bypassed.
