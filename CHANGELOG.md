...
- `core.telemetry`: **`instrument()` now installs the DataRobot span exporter wherever OTel export is configured, not only in a deployment or workload.** The `MLOPS_DEPLOYMENT_ID` / `WORKLOAD_ID` gate had dropped every framework, LLM, HTTP, memory, and moderation span everywhere else — including custom applications, which the platform gives `OTEL_EXPORTER_OTLP_HEADERS` but no deployment id, and local `dr agent run`. Those now export. It stays a no-op when nothing configures export.
- `core.telemetry`: new `trace_to_use_case("<name>")` starts local tracing in one call: it picks the use case (`DATAROBOT_USE_CASE_ID`, `DATAROBOT_DEFAULT_USE_CASE`, or one of that name, created on first use), calls `instrument()`, and reports where spans went rather than raising. A local run can also just set `DATAROBOT_USE_CASE_ID`, which resolves to the `experiment_container-<id>` entity id behind a deployment or workload id.
- `core.telemetry`: a malformed `OTEL_EXPORTER_OTLP_HEADERS` entry is skipped instead of raising, which the gate removal above would otherwise surface as a crash at `register.py` import.
