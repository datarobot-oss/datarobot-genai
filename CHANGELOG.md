...
## 0.30.0
- `core/config`: **an agent can reach many MCP servers at once.** Each is configured by its own prefixed variables — `WEATHER_MCP_DEPLOYMENT_ID`, `ANALYTICS_MCP_WORKLOAD_ID`, `SEARCH_MCP_URL`, `DOCS_MCP_LOCAL_PORT`, with a short form without `_MCP`. Applications name no server: one `mcp_servers` field plus `include_mcp_servers_settings_source`, and the source discovers whatever exists across runtime parameters, environment, `.env`, file secrets and `pulumi_config.json`. Adding a server is two lines in `.env`.
- `core/mcp`: **how a server is addressed no longer decides how it is authenticated.** `<name>_MCP_AUTH_PROVIDER` names an `authentication:` entry; leaving it unset reproduces the previous behavior from the address kind. A DataRobot-hosted server addressed by `url` can now carry a token — which is what makes the platform's global MCP reachable — and any server can be reached anonymously.
- `core/mcp`: a `url` server on a host other than the `DATAROBOT_ENDPOINT` host cannot receive DataRobot credentials, and there is no override. Set `<name>_MCP_AUTH_PROVIDER=none`, or address it by workload or deployment ID so the URL is derived rather than asserted.
- `core/mcp`: workload-hosted MCP servers are first-class and authenticated. Their URL is composed from `DR_WORKLOAD_EXTERNAL_URL_HOST`/`_PREFIX` rather than looked up, dropping one 10-second HTTP call per workload along with the endpoint cache and its concurrent fan-out.
- `dragent`: MCP clients are built once at workflow build and stay connected; the auth provider recomputes credentials per request.
...
- *Breaking change*: `authorization_context` and `forwarded_headers` are no longer fields — as fields on a shared object they made it unsafe to reuse across requests. **Pass them as arguments**: `mcp_tools_context(target, forwarded=..., auth_context=...)`.
- *Breaking change*: the LangGraph, CrewAI and LlamaIndex `mcp_tools_context` adapters take an `MCPTarget` rather than an `MCPConfig`, and namespace each server's tools `<server name>__<tool>` to match NAT's function groups. Without the prefix two servers each exposing `search` collide. **Update prompts, evals and per-tool guardrails once**; `prefix=""` keeps raw names, safe only with a single server.
- *Breaking change*: `MCP_DEPLOYMENT_ID`, `MCP_WORKLOAD_ID` and `EXTERNAL_MCP_URL` are now mutually exclusive, as the 0.28.1 notes already described them. Each still works alone as the server named `default`, so no existing `.env` breaks; setting two now raises instead of silently discarding one. `MCP_SERVER_PORT` is exempt — it names the port the bundled MCP **server** binds, and the templates set it unconditionally.
- *Breaking change*: a configured but unreachable server now raises instead of yielding an empty tool list, which was indistinguishable from "no MCP configured". An unresolvable address raises instead of falling back to `http://localhost:8080/mcp`, and a malformed deployment or workload ID fails at config load. Pass `strict=False` to the adapters for the old behavior.
- `core/config`: `MCP_SERVERS` and the per-server variables are now unioned. `MCP_SERVERS` silently outranked them, and a leftover `MCP_SERVERS=[]` emptied the fleet outright. Defining one name in both raises.
- `dragent`: a `workflow.yaml` block with an inline `url` and no `name` no longer silently replaces a `default` set through `MCP_DEPLOYMENT_ID`, `MCP_WORKLOAD_ID` or `EXTERNAL_MCP_URL`; it raises. Replacing the `MCP_SERVER_PORT` default is still allowed.
- `core/config`: a malformed `MCP_SERVERS` reports the JSON parse error and the offending value, not just `error parsing value for field "mcp_servers"`.
- `core/config`: `DATAROBOT_GENAI_MAX_HISTORY_MESSAGES` is now readable as a runtime parameter; the field used `alias=`, which the runtime-parameter source does not key on. The environment variable name is unchanged.
- `core/config`: an empty `MCP_DEPLOYMENT_ID=` left in a container image no longer outranks the runtime parameter infra deliberately set (`env_ignore_empty`).
