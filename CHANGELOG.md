...
## 0.30.0
- **An agent can now reach as many MCP servers as it needs, of every kind, at once.** Declare them in one variable, `MCP_SERVERS`: a JSON array where each entry is a name plus exactly one address — `deployment_id`, `workload_id`, `local_port` or `url`. Several deployments, several workloads, several local processes and third-party servers can be mixed freely in one run, and each server carries its own credentials. The same `.env` works on a laptop and deployed; infra emits the same variable as a runtime parameter.
