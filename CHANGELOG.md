...
- `crewai`: the router LLM reports `supports_function_calling` from its whole failover chain, not just the primary. The router fails over primary → fallbacks at runtime, so a placeholder/retired primary no longer forces the inferior ReAct path (where a model can hallucinate the tool result) when a reachable fallback supports tools.
