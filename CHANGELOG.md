...
- `drmcpbase/oauth_scopes`: removed the tool-level OAuth enforcement added in 0.27.3 — the per-component checks that verified the request's own bearer token and hid components from callers lacking scopes. Authorization is left to the existing ASGI middleware (`drmcp/core/middleware`), which validates the audience claim and checks each `tools/call` against the tool's declared scopes via `declared_scopes_for_one_tool`. The module is declaration-only now: declarations record scopes on components without gating callers, and tools are no longer hidden from `tools/list`. Enforcement is therefore gated by `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` alone — setting `MCP_OAUTH_AUTHORIZATION_SERVERS` no longer activates verification by itself.
  ...
  - `wire_scopes` now refuses to start a server whose components carry auth checks not produced by `required_scopes`/tag rules — FastMCP's own `require_scopes`, for example: `report_foreign_auth_checks` logs the offending components at ERROR and raises the new `ForeignAuthCheckError`. FastMCP evaluates those checks at the tool level against `ctx.token`, which it reads from the `request.scope["user"]` our token handler sets only while `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` is on: gate off, the component is hidden from every caller; gate on, it is hidden from callers short of the scope instead of refused with 403. Either way its scopes never reach the middleware, `scopes_supported` or the REST `required_scopes` field.
