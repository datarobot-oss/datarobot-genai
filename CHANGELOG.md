...
## 0.28.4
- `drtools/core/sandbox`: the marker wait no longer gives up on a container that is still running. 0.28.2 required stdout to stay unchanged for 5s before declaring a run markerless, but stdout can sit frozen at the runner's first line for longer than that while the OTEL collector flushes the marker — observed on staging holding at 97 bytes across three polls (~10s), with the marker available 3s after we gave up, at 17s of a 324s budget. The decision is now keyed on whether the container produced ANY output rather than on how long output has been still: the image runner prints its marker after its own `try/except BaseException/finally`, so any container that started emits one on every path, and non-empty output therefore means a marker is in flight. Empty output (container likely never started) still fails fast at 5s; non-empty output waits, bounded at 30s of stillness to cover a container SIGKILLed before the print.
