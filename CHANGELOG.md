...
## 0.19.2
- `nat/dr_mem0_memory`: adapt to the DataRobot Memory Service no longer shipping a default LLM model name (memory-service commit `1d7acd8e`). `DRMem0MemoryClientConfig` gains a required `llm_model_name` field (also readable from the `AGENT_LLM_MODEL_NAME` env var). A Pydantic validator raises `ValueError` at config construction time when `agent_memory_space_id` is set without `llm_model_name`, failing fast instead of silently reaching the service only to get HTTP 400. When set, the provider PATCHes the memory space with the model name at editor-init time. Integration tests updated: `dr_memory_space` fixture now passes `llm_model_name="gpt-4o"` to `MemorySpace.create`, and a new test (`test_dr_mem0_config_llm_model_name_patches_memory_space`) verifies the PATCH path end-to-end.
