...
## 0.29.41
- `drmcpbase/oauth_scopes`: removed the tool-level OAuth enforcement added in 0.27.3 — the per-component checks that verified the request's own bearer token and hid components from callers lacking scopes. Authorization is left to the existing ASGI middleware (`drmcp/core/middleware`), which validates the audience claim and checks each `tools/call` against the tool's declared scopes via `declared_scopes_for_one_tool`. The module is declaration-only now: declarations record scopes on components without gating callers, and tools are no longer hidden from `tools/list`. Enforcement is therefore gated by `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` alone — setting `MCP_OAUTH_AUTHORIZATION_SERVERS` no longer activates verification by itself.
  - `declared_scopes_for_one_tool` now honours the scope source, so a declaration silenced by `MCP_OAUTH_SCOPE_SOURCE` (e.g. in-code scopes under `tags`) is no longer enforced by the middleware either.
  - Removed with the tool-level path: `require_scopes`, `require_verified_token`, `apply_token_floor`, `probe_verification_keys`, `request_scopes`, `satisfies`, the `JWTVerifier`-based bearer-token reading (with its per-request memoisation and the startup JWKS fetch), `ScopeSettings`' `issuer`/`audience`/`jwks_uri`/`enforced`, the `MCP_OAUTH_JWKS_URI` setting, and the startup warnings about a partially configured verifier or multiple authorization servers — `drmcp/core/oauth_scopes.build_scope_settings` no longer reads any of those settings.
  - `wire_scopes` now warns at startup about components carrying auth checks not produced by `required_scopes`/tag rules — FastMCP's own `require_scopes`, for example (`report_foreign_auth_checks`). FastMCP evaluates those at the tool level against `ctx.token`, which it reads from the `request.scope["user"]` our token handler sets only while `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` is on: gate off, the component is hidden from every caller; gate on, it is hidden from callers short of the scope instead of refused with 403. Either way its scopes never reach the middleware, `scopes_supported` or the REST `required_scopes` field.
- `drmcp`: OAuth scopes are declared as plain data, `required_scopes=(...)`, on the tool's own decorator — `@dr_mcp_tool(tags={"database"}, required_scopes=("mcp:tools:execute",))` for user tools, `@tool_metadata(required_scopes=(...))` for drtools tools (which may not import drmcpbase/fastmcp — see `scripts/check_imports.py`). `dr_mcp_tool` converts the key into a declaration check (stacking with any `auth` given, never clobbering it); a lone scope may be written as a bare string. `ToolKwargs` gained `required_scopes`, and the FastMCP-facing subset it extends is now its own TypedDict, `MCPToolKwargs`. `require_scopes(...)` is removed — from `drmcpbase.oauth_scopes` and from the `datarobot_genai.drmcp` export — migrate `auth=require_scopes("a", "b")` to `required_scopes=("a", "b")`. `required_scopes` also joins `drmcputils.tool_gallery.DRTOOLS_PRIVATE_METADATA_KEYS` so registrars that do not convert it strip it instead of passing an unknown kwarg to `mcp.tool()`.
- `drmcp/core/oauth_scopes.wire_scopes`: warns at startup when scopes are declared but `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` is off — the gate disables the token, audience and scope middlewares together, so declared requirements would otherwise be published in `scopes_supported` while every `tools/call` passes, silently.
- `drmcp/core/middleware`: audience validation no longer requires Cross-Application Access. `GeneralOAuthClaimValidationMiddleware` expects `MCP_XAA_TOKEN_AUDIENCE` when set, else `MCP_OAUTH_AUDIENCE`.
  - The audience and tool-scope validators now return 401 `invalid_token` when the request carries no authenticated user, instead of passing it through. Unreachable in the shipped middleware order — `OAuthJWTTokenHandlerMiddleware` runs first and either sets the user or returns 401 itself — but with tool-level enforcement gone the validators are the only check, so they no longer depend on that order. Nothing changes with `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` off: the gate short-circuits before either validator runs.
- `drmcpbase/routes/static`: each `/static/tools/` item now carries `required_scopes` — the OAuth scopes a `tools/call` token must cover, combined across both declaration spellings (`required_scopes` on the tool, `MCP_OAUTH_TAG_SCOPES_<TAG>`), honouring `MCP_OAUTH_SCOPE_SOURCE`. Read with the new `drmcpbase.oauth_scopes.declared_scopes_of_component` — the same reader the scope-validation middleware enforces with (`declared_scopes_for_one_tool` now delegates to it), so what the API reports is exactly what is enforced. `merge_tool_info` grew an optional `scopes_provider` argument for callers outside drmcpbase.
