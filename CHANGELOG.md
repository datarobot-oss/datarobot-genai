...
## 0.26.21
- `dragent`: A2A public agent cards are now always redacted. The `agent/getAgentCard` endpoint returns a safe discovery view — skills are stripped and internal/external identity extensions are removed — while auth and cross-application-access metadata stay advertised. Authenticated callers receive the full card through `agent/getAuthenticatedExtendedCard` (`supports_authenticated_extended_card=True`).
