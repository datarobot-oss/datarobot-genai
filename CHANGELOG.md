...
- `drmcputils/categories`: `TOOL_CATEGORY_LABELS` now covers **every** `MCPToolCategory` member, children included, and `category_label()` resolves one (falling back to the raw name). A test pins full coverage, so adding a category without a label fails CI instead of shipping a node labelled with its own `dr_*` string. Which categories are *filterable* is decided by the taxonomy's shape (its top-level nodes) rather than by a hand-kept subset that can fall behind it.
...
- `drmcputils/routes`: **`TrailingSlashNormalizer` now serves every registered route with or without its trailing slash**, on both servers. An ASGI scope rewrite, never a 307 — a redirect's `Location` is built from the path the *container* saw and does not resolve back through the deployment directAccess gateway, so it dead-ends instead of correcting. This replaces `drmcp`'s `MCPTrailingSlashNormalizer`, which covered only the MCP mount and lived in a package global-mcp imports nothing from: global-mcp had no normalization at all, and on both servers `/toolGallery/tools`, `/toolGallery/categories`, `/metadata/` and `/toolGallery/toolSets/{id}` still redirected. Rules are `(prefix, canonical trailing slash)`, matched longest-prefix-first; the prefix form is what covers parameterized routes. Build them with `default_slash_rules(prefix_mount_path)` — passing the caller's own prefixing callable is what makes the rules match whether the server is mounted under a `URL_PREFIX` (the deployment-backed shape) or not (the workload-backed shape, and global-mcp). `/metadata` stays registered bare to match user-mcp's inline route; with normalization that asymmetry is no longer something callers have to know.
