...
## 0.29.41
- `drmcpbase/oauth_scopes`: removed the tool-level OAuth enforcement added in 0.27.3 — the per-component checks that verified the request's own bearer token and hid components from callers lacking scopes. Authorization is left to the existing ASGI middleware (`drmcp/core/middleware`), which validates the audience claim and checks each `tools/call` against the tool's declared scopes via `declared_scopes_for_one_tool`. The module is declaration-only now: `require_scopes(...)` — still `@dr_mcp_tool(auth=require_scopes(...))`, exported from `datarobot_genai.drmcp` — records scopes on the component without gating callers, and tools are no longer hidden from `tools/list`. Enforcement is therefore gated by `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` alone — setting `MCP_OAUTH_AUTHORIZATION_SERVERS` no longer activates verification by itself.
  ...
  - Removed with the tool-level path: `require_verified_token`, `apply_token_floor`, `probe_verification_keys`, `request_scopes`, `satisfies`, the `JWTVerifier`-based bearer-token reading (with its per-request memoisation and the startup JWKS fetch), `ScopeSettings`' `issuer`/`audience`/`jwks_uri`/`enforced`, the `MCP_OAUTH_JWKS_URI` setting, and the startup warnings about a partially configured verifier or multiple authorization servers — `drmcp/core/oauth_scopes.build_scope_settings` no longer reads any of those settings.
- `drmcp`: `dr_mcp_tool` / `dr_core_mcp_tool` wrap every check passed on `auth=` so it admits every caller while `MCP_ENABLE_OAUTH_CLAIM_VALIDATION` is off (the flag is read when the check runs, as the middleware reads it) and runs unchanged when it is on; nothing is removed. This is for FastMCP-native checks (`fastmcp.server.auth.require_scopes`, `restrict_tag`, custom checks): FastMCP evaluates them at the tool level against `ctx.token`, which it reads from the `request.scope["user"]` our token handler sets only while the gate is on — so with the gate off no token reaches FastMCP and such a check would hide the tool from every caller. A warning at registration names tools carrying one. With the gate on they still gate at the tool level (hiding the tool rather than answering 403, invisible to `scopes_supported` and the REST `required_scopes` field) — declare scopes with `datarobot_genai.drmcp.require_scopes` instead, which keeps its declaration marker through the wrapper.
