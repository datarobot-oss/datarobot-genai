...
## 0.15.89
- `dragent/frontends/step_adaptor.py`: normalize list-form reasoning content so the DRAgent/NAT step adaptor no longer crashes when a reasoning model streams `[{"type": "thinking", ...}, {"type": "text", ...}]` into a string `delta`. Root-level thinking is emitted as AG-UI `ReasoningMessageChunkEvent`s and text as `TextMessageContentEvent`s; nested (sub-agent) output stays in the reasoning envelope. Reuses the `core/agents/reasoning` block helpers.
- `core/agents/reasoning.py`: `_iter_message_blocks` no longer double-emits reasoning when the DataRobot gateway returns the same delta in both `additional_kwargs["reasoning_content"]` and native `content` thinking blocks — the flat `reasoning_content` is only used as a fallback when `content` does not already carry thinking.
- `nat/agent.py`: reverted the `_extract_reasoning` forwarding added in 0.15.88. NAT's `tool_calling_agent` stream drops reasoning upstream (only string `content` + `tool_calls` are forwarded), so the forwarding was never exercised; NAT stays text-only.
