...
- `dragent/frontends/step_adaptor.py`: normalize list-form reasoning content so the DRAgent/NAT step adaptor no longer crashes when a reasoning model streams `[{"type": "thinking", ...}, {"type": "text", ...}]` into a string `delta`. Root-level thinking → `ReasoningMessageChunkEvent`s, text → `TextMessageContentEvent`s; nested (sub-agent) output stays in the reasoning envelope.
- `nat/agent.py`: NAT remains text-only — its `tool_calling_agent` stream forwards only string `content` + `tool_calls` and drops `reasoning_content` upstream (`ChatResponseChunk` has no reasoning field), so reasoning can't be surfaced for NAT without an upstream change.
