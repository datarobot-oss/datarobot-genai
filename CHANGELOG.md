...
## 0.26.21
- `dragent`: A2A agent cards are now redacted for anonymous callers. The public `agent/getAgentCard` endpoint returns a safe discovery view — skills are stripped and internal/external identity extensions are removed — while auth and cross-application-access metadata stay advertised. Same-tenant authenticated callers (via `X-DataRobot-Authorization-Context` or `X-DataRobot-User-Id`) receive the full card; the extended card is also available through `agent/getAuthenticatedExtendedCard` (`supports_authenticated_extended_card=True`). An invalid or expired auth-context JWT on the public card route returns 403 instead of falling through to the redacted view.
