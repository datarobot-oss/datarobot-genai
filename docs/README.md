...
The standalone CLI runs and queries DRAgent workflows over NAT. See [docs/dragent/](dragent/) for `serve`, `run`, `query`, completion JSON, authentication, and debugging. For wiring OpenTelemetry tracing into the deployment **Tracing** tab, see [docs/dragent/tracing.md](dragent/tracing.md).
