...
Use **`get_datarobot_gateway_llm()`** when you always want the gateway. Calls go through the gateway URL derived from `DATAROBOT_ENDPOINT`; use `DATAROBOT_API_TOKEN` as the API key.
...
Use **`get_external_llm()`** when you want LiteLLM to call **external** providers (for example OpenAI) using **their** environment variables (for example `OPENAI_API_KEY`). Do **not** use the `datarobot/` prefix for external model names.
...
To reach this route via **`get_llm()`**, leave `LLM_DEPLOYMENT_ID` and `NIM_DEPLOYMENT_ID` unset and use a model name without the `datarobot/` prefix (see **get_llm()** below).
...
1. **Deployment** if `LLM_DEPLOYMENT_ID` is set.
2. Else **NIM** if `NIM_DEPLOYMENT_ID` is set.
3. Else **Gateway** if `LLM_DEFAULT_MODEL` starts with `datarobot/`.
4. Else **external** (LiteLLM using provider-specific environment variables).

If both `LLM_DEPLOYMENT_ID` and `NIM_DEPLOYMENT_ID` are set, **deployment wins**.

These variables control **`get_llm()`** specifically:

| Variable | Role |
|---|---|
| `LLM_DEPLOYMENT_ID` | Use this **LLM deployment** chat endpoint. |
| `NIM_DEPLOYMENT_ID` | When no LLM deployment id is set, use this **NIM** deployment. |
| `LLM_DEFAULT_MODEL` | Default model id when you omit `model_name`. Prefix with `datarobot/` to route via the **DataRobot LLM Gateway**. |
...
That component follows the same routing logic using fields on the block (model name prefix, deployment ids, etc.) and the environment.
