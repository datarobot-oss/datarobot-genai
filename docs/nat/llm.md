...
| **`datarobot-llm-router`** | Primary + fallback LLMs with automatic failover via LiteLLM Router; includes `primary`, `fallbacks`, and optional tuning fields (`allowed_fails`, `cooldown_time`). |
