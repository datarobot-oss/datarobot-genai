...
Enable both NAT lifecycle and framework spans for full coverage; mem0 spans appear automatically when memory is configured and tracing is enabled.
...
When the OTLP vars are not set, the runtime **falls back** to deriving the endpoint and headers from the DataRobot deployment env (populated automatically inside a deployment):
...
- **`datarobot_entity_id must be of the form 'deployment-<id>'`**: `datarobot_entity_id` was set manually without the `deployment-` prefix. Either add the prefix or omit the field inside a deployment — it auto-derives from `MLOPS_DEPLOYMENT_ID`.
