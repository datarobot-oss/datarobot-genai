...
On enclave workloads (both `DR_WORKLOAD_EXTERNAL_URL_HOST` and `DR_WORKLOAD_EXTERNAL_URL_PREFIX` set, with `WORKLOAD_ID` injected by the platform), dragent provisions a shared MemorySpace for the L2 cache when `AGENT_CARD_REGISTRY_MEMORY_SPACE_ID` is unset and the workflow uses central registry lookups (`registry` on `authenticated_a2a_client` function groups). The space is keyed to `WORKLOAD_ID` via a `deduplication_key` so replicas share one cache without infra wiring. Other hosted runtimes should set `AGENT_CARD_REGISTRY_MEMORY_SPACE_ID` via Pulumi / `task deploy-dev`. Locally, only in-process L1 caching is used unless you set `AGENT_CARD_REGISTRY_MEMORY_SPACE_ID` yourself. This cache is separate from agent memory (`AGENT_MEMORY_SPACE_ID`), which is provisioned on the control hub.
...
| `AGENT_CARD_REGISTRY_MEMORY_SPACE_ID` | No | MemorySpace ID for the registry L2 cache. Auto-provisioned on enclave workloads when unset and registry-backed A2A clients are configured; set via Pulumi / `task deploy-dev` on other hosted runtimes; optional override for local testing. |
