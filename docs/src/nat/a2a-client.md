...
While the server is running, registered cards are **refreshed in the background** on an interval equal to the soft cache TTL. Only entries past the soft TTL are re-fetched; failures are logged and existing cache entries are retained. If a registry fetch fails, the last-known-good cached card is served.
