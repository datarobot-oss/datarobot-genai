...
def convert_to_pipeline_messages(
    ...
    return [_to_message(step) for step in steps if _include_step(step)]
...
class NatAgent(BaseAgent[None]):
    ...
    def create_pipeline_interactions_from_steps(
        ...
        trace = convert_to_pipeline_messages(steps)
        return MultiTurnSample(user_input=trace)
