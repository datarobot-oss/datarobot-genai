...
from datarobot_genai.application_utils.chat_history.repositories import _BoundedLRU
...
async def test_create_chat_is_idempotent_by_user_thread(env: Env) -> None:
    ...
    # No duplicate *chat* session created (the space also holds the //loc/ locator).
    chat_sessions = [
        s for s in env.svc.sessions.values() if (s.get("description") or "").startswith("//thread/")
    ]
...
async def test_update_tool_call_discovers_on_cold_cache(env: Env) -> None:
    """GIVEN a fresh repo WHEN updating a tool call THEN its parent resolves via the locator."""
    chat = await _make_chat(env)
    msg = await env.message_repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="", chat_id=chat.chat_uuid)
    )
...
async def test_update_reasoning_discovers_on_cold_cache(env: Env) -> None:
    """GIVEN a fresh repo WHEN updating a reasoning THEN its parent resolves via the locator."""
    chat = await _make_chat(env)
    msg = await env.message_repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="", chat_id=chat.chat_uuid)
    )
...
async def test_get_message_discovers_on_cold_cache(env: Env) -> None:
    """GIVEN a fresh repo/registry WHEN getting a message THEN it resolves via the locator."""
    chat = await _make_chat(env)
    msg = await env.message_repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="hi", chat_id=chat.chat_uuid)
    )

    # A brand-new registry and repository have empty caches — force the locator path.
    ...
    # Resolving the message:<uuid> locator registered the chat → session mapping too.
...
async def test_registry_resolve_via_locator_on_miss(env: Env) -> None:
    """GIVEN an unseen chat WHEN resolving THEN the chat:<uuid> locator finds and caches it."""
    chat = await _make_chat(env)
    cold_registry = ChatSessionRegistry(env.space)
...
# ── EntityLocator secondary index ───────────────────────────────────────────────


async def test_create_paths_write_locators(env: Env) -> None:
    """GIVEN a chat/message/tool-call/reasoning WHEN created THEN each writes its locator."""
    user = uuid4()
    chat = await env.chat_repo.create_chat(ChatCreate(name="C", thread_id="t", user_uuid=user))
    msg = await env.message_repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="hi", chat_id=chat.chat_uuid)
    )
    tc = await env.message_repo.create_message_tool_call(
    ...
    rs = await env.message_repo.create_message_reasoning(
        MessageReasoningCreate(message_uuid=msg.message_uuid, content="think")
    )

    index = LocatorIndex(env.space)
    sid = _session_id(env, chat.chat_uuid)

    chat_loc = await index.get(LOCATOR_KIND_CHAT, chat.chat_uuid)
    assert chat_loc is not None
    assert chat_loc.session_id == sid
    assert chat_loc.chat_uuid == chat.chat_uuid
    assert chat_loc.message_uuid is None

    msg_loc = await index.get(LOCATOR_KIND_MESSAGE, msg.message_uuid)
    assert msg_loc is not None
    assert msg_loc.session_id == sid
    assert msg_loc.chat_uuid == chat.chat_uuid

    tc_loc = await index.get(LOCATOR_KIND_TOOL_CALL, tc.uuid)
    assert tc_loc is not None
    assert tc_loc.message_uuid == msg.message_uuid
    assert tc_loc.chat_uuid == chat.chat_uuid

    rs_loc = await index.get(LOCATOR_KIND_REASONING, rs.uuid)
    assert rs_loc is not None
    assert rs_loc.message_uuid == msg.message_uuid


async def test_locators_are_isolated_from_chat_listing(env: Env) -> None:
    """GIVEN locators sharing the space WHEN listing chats THEN locators are not returned.

    The ``//loc/`` description prefix keeps :class:`EntityLocator` sessions out of
    every ``Chat.list`` result (the ORM prefix-scoping fix), so ``get_all_chats``
    counts only real chats even though the space also holds their locators.
    """
    user = uuid4()
    await env.chat_repo.create_chat(ChatCreate(name="A", thread_id="a", user_uuid=user))
    await env.chat_repo.create_chat(ChatCreate(name="B", thread_id="b", user_uuid=user))

    # Space holds 2 chats + 2 chat locators, but only the chats are listed.
    assert len(env.svc.sessions) == 4
    assert len(await env.chat_repo.get_all_chats(None)) == 2


async def test_lost_message_locator_makes_message_unaddressable_cross_replica(env: Env) -> None:
    """GIVEN a lost message-locator write WHEN a cold replica looks it up THEN it returns None.

    Documents the accepted consistency trade-off: the message event is the source
    of truth (``get_chat_messages`` still returns it), but with the best-effort
    locator write lost, a *different* replica cannot address the message by uuid.
    """
    user = uuid4()
    chat = await env.chat_repo.create_chat(ChatCreate(name="C", thread_id="t", user_uuid=user))

    env.svc.fail_locator_write = True  # this replica cannot write the message locator
    msg = await env.message_repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="hi", chat_id=chat.chat_uuid)
    )
    env.svc.fail_locator_write = False

    # The create still succeeded — the writing worker addresses it from its warm cache.
    warm = await env.message_repo.get_message(msg.message_uuid)
    assert warm is not None
    assert warm.content == "hi"

    # A cold replica has no message:<uuid> locator → cannot resolve by uuid.
    cold_repo = MessageRepository(env.space, ChatSessionRegistry(env.space))
    assert await cold_repo.get_message(msg.message_uuid) is None

    # …but the message is NOT lost: listing the chat's events still returns it
    # (the chat:<uuid> locator resolves the chat; its events are read directly).
    cold_repo2 = MessageRepository(env.space, ChatSessionRegistry(env.space))
    listed = await cold_repo2.get_chat_messages(chat.chat_uuid)
    assert [m.content for m in listed] == ["hi"]


async def test_evicted_message_cache_entry_still_resolves_via_locator(env: Env) -> None:
    """GIVEN a bounded cache that evicted a message WHEN getting it THEN the locator resolves it."""
    chat = await _make_chat(env)
    repo = env.message_repo
    repo._msg_chat = _BoundedLRU(maxsize=1)  # only the newest message stays cached

    m1 = await repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="one", chat_id=chat.chat_uuid)
    )
    await repo.create_message(
        MessageCreate(role=Role.ASSISTANT.value, content="two", chat_id=chat.chat_uuid)
    )

    # m1's cache entry was evicted by m2; the lookup must fall back to the locator.
    assert repo._msg_chat.get(m1.message_uuid) is None
    found = await repo.get_message(m1.message_uuid)
    assert found is not None
    assert found.content == "one"
...
def test_bounded_lru_evicts_least_recently_used() -> None:
    """GIVEN a full cache WHEN inserting past the cap THEN the least-recently-used is evicted."""
    cache: _BoundedLRU[int, int] = _BoundedLRU(maxsize=2)
    cache[1] = 10
    cache[2] = 20
    cache[3] = 30  # evicts key 1 (oldest, never re-accessed)
...
def test_bounded_lru_access_bumps_recency() -> None:
    """GIVEN a full cache WHEN a key is read THEN it survives the next eviction."""
    cache: _BoundedLRU[int, int] = _BoundedLRU(maxsize=2)
    cache[1] = 10
    cache[2] = 20
    assert cache.get(1) == 10  # bump key 1 → key 2 becomes least-recently-used
    cache[3] = 30  # evicts key 2, not key 1
...
def test_bounded_lru_pop_discards() -> None:
    """GIVEN a cached key WHEN popped THEN it is gone and pop returns a default on a miss."""
    cache: _BoundedLRU[int, int] = _BoundedLRU(maxsize=4)
    cache[1] = 10
    assert cache.pop(1) == 10
