...
class TestAgentConversationWorkflow:
    ...
    async def test_agent_conversation_memory_persistence(self, test_storage_path: str):
        """
        ...
        """
        # This test uses ResourceStore directly (not MCP session)
...
class TestAgentMemoryWorkflow:
    ...
    async def test_agent_memory_write_read_via_mcp(self, test_storage_path: str):
        """
        ...
        try:
            ...
            async with integration_test_mcp_session() as session:
                ...
                memory_write_tool = next(
                    (t for t in tools if t.name == "memory_write"), None
                )
    ...
    async def test_agent_memory_search_via_mcp(self, test_storage_path: str):
        """
        ...
        try:
            ...
            async with integration_test_mcp_session() as session:
                ...
                memory_search = next(
                    (t for t in tools_result.tools if t.name == "memory_search"), None
                )
                ...
                search_content = (
                    search_result.content[0].text if search_result.content else ""
                )
    ...
    async def test_agent_memory_delete_via_mcp(self, test_storage_path: str):
        ...
        try:
            ...
            async with integration_test_mcp_session() as session:
                ...
                # Delete the memory
                memory_delete = next(
                    (t for t in tools_result.tools if t.name == "memory_delete"), None
                )
...
class TestAgentResourceWorkflow:
    ...
    async def test_agent_store_and_retrieve_resource_via_mcp(self, test_storage_path: str):
        """
        ...
        try:
            ...
            async with integration_test_mcp_session() as session:
                ...
                our_resource = next(
                    (r for r in resources_result.resources if resource_id in str(r.uri)),
                    None,
                )
