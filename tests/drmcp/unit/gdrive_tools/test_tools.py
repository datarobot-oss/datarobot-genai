...
def gdrive_client_list_files_with_next_page_mock(
    ...
    with patch("datarobot_genai.drtools.core.clients.gdrive.GoogleDriveClient.list_files") as mock:
...
class TestGdriveListFiles:
    ...
    async def test_gdrive_find_contents_when_next_page_available_happy_path(
        ...
        result = await gdrive_find_contents(fields=["id", "name"])
        content = result
        expected = {
...
class TestGdriveReadContent:
    ...
    async def test_gdrive_read_content_happy_path(
        ...
        assert content["id"] == "doc123"
        assert content["name"] == "My Document"
        assert content["mimeType"] == "text/markdown"
        assert content["content"] == "# Hello World\n\nThis is content."
        assert content["originalMimeType"] == "application/vnd.google-apps.document"
        assert content["wasExported"] is True
        assert content["size"] == 1024
        assert content["webViewLink"] == "https://docs.google.com/document/d/doc123/edit"
        assert content["wasExported"] is True
...
class TestGdriveCreateFile:
    ...
    async def test_gdrive_create_file_happy_path(
        ...
        with patch(
            ...
            result = await gdrive_create_file(name="My New File.txt", mime_type="text/plain")
        content = result
        # Verify content is properly returned
        assert content["id"] == "new_file_123"
        assert content["name"] == "My New File.txt"
    ...
    async def test_gdrive_create_file_with_content(
        ...
        with patch(
            ...
            mock.return_value = created_file
            result = await gdrive_create_file(
    ...
    async def test_gdrive_create_google_doc(
        ...
        content = result
        assert content["mimeType"] == "application/vnd.google-apps.document"
        assert content["mimeType"] == "application/vnd.google-apps.document"
    ...
    async def test_gdrive_create_folder(
        ...
        content = result
        assert content["mimeType"] == GOOGLE_DRIVE_FOLDER_MIME
        assert content["mimeType"] == GOOGLE_DRIVE_FOLDER_MIME
...
class TestGdriveManageAccess:
    ...
    async def test_gdrive_add_role_happy_path(self, get_gdrive_access_token_mock: None) -> None:
        ...
        with patch(
            ...
            result = await gdrive_manage_access(
                file_id="dummy_file_id", action="add", role="reader", email_address="dummy@user.com"
            )
        content = result
        assert content["affectedFileId"] == "dummy_file_id"
        assert content["newPermissionId"] == new_permission_id
        assert content["affectedFileId"] == "dummy_file_id"
        assert content["newPermissionId"] == new_permission_id
...
def gdrive_client_update_file_metadata_mock() -> Iterator[GoogleDriveFile]:
    with patch(
        "datarobot_genai.drtools.core.clients.gdrive.GoogleDriveClient.update_file_metadata"
    ) as mock:
...
class TestGdriveUpdateMetadata:
    ...
    async def test_gdrive_update_metadata_happy_path(
        ...
        result = await gdrive_update_metadata(
            file_id="file_123", new_name="Updated File.txt", starred=True, trash=False
        )
        content = result
        assert content["name"] == "Updated File.txt"
        assert content["starred"] is True
        assert content["id"] == "file_123"
        assert content["name"] == "Updated File.txt"
        assert content["starred"] is True
