...
def patch_realtime_dependencies() -> Generator[dict[str, Any], None, None]:
    ...
    with (
        patch(
            "datarobot_genai.drtools.predictive.predict_realtime.dr_client",
        ) as mock_dr_client,
...
@pytest.mark.asyncio
async def test_predict_realtime_forecast_range_oversize_raises(
    patch_realtime_dependencies: dict[str, Any],
) -> None:
...
async def test_predict_timeseries_regression_forecast_point_with_intervals(
    ...
    assert (
        "1425.5" in content["data"]
    )  # Check regression prediction value (pandas removes trailing zeros)
    assert "prediction_lower" in content["data"]
    assert "prediction_upper" in content["data"]
...
async def test_predict_timeseries_regression_multiseries(
    ...
    assert "1425.5" in content["data"]  # Store A prediction (pandas removes trailing zeros)
    assert "1125.75" in content["data"]  # Store B prediction
...
@pytest.mark.asyncio
async def test_predict_timeseries_regression_large_dataset_raises(
    patch_realtime_dependencies: dict[str, Any],
) -> None:
    """Large realtime prediction results raise ToolError instead of offloading to object storage."""
...
async def test_predict_timeseries_regression_no_prediction_intervals(
    ...
    assert "prediction_lower" not in content["data"]
    assert "prediction_upper" not in content["data"]
...
class TestPredictByAiCatalogRt:
    ...
    async def test_predict_by_ai_catalog_rt_with_get_as_dataframe(
        ...
        pr_args, pr_kwargs = mock_predictions_result_response.call_args
        assert pr_args[0].equals(mock_result.dataframe)
        assert pr_kwargs.get("show_explanations") is True
