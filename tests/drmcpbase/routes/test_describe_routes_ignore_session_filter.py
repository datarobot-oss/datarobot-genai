...
``/metadata`` answers "what can this server do", and UIs build their pickers from it — a
taxonomy already narrowed by the caller's own filter cannot offer the categories that
filter excluded.
...
class TestDescribeRoutesIgnoreTheSessionFilter:
    def test_an_allowlist_header_does_not_narrow_the_catalog(self) -> None:
        """GIVEN a caller allowlisted to one tool, THEN metadata still reports both."""
        ...
        assert unfiltered["metadata"] == ["jira_search_issues", "perplexity_search"]
    ...
    def test_an_unsupported_or_collapsing_mode_header_does_not_change_the_answer(
        ...
        """``mode=search`` used to zero the counts; ``mode=code`` used to 500."""
        with TestClient(_server(unfiltered=True).http_app()) as client:
            resp = client.get("/metadata", headers={"x-datarobot-mcp-mode": mode})

        assert resp.status_code == 200
        assert len(resp.json()["tools"]["items"]) == 2
    ...
    def test_metadata_answers_json_when_the_catalog_cannot_be_built(self) -> None:
        """A transform that rejects the request must not escape as a bare 500."""
        with TestClient(_server(unfiltered=False).http_app()) as client:
            resp = client.get("/metadata", headers={"x-datarobot-mcp-mode": "code"})
