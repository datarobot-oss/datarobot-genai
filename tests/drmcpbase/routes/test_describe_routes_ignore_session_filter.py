...
``/static/tools/`` and ``/static/categories/`` answer "what can this server do", and UIs
build their pickers from them — a taxonomy already narrowed by the caller's own filter
cannot offer the categories that filter excluded.
...
def _server(*, unfiltered: bool) -> FastMCP:
    ...
    register_static_routes(mcp, catalog_provider=provider)
    return mcp
...
def _tool_names(client: TestClient, headers: dict[str, str]) -> dict[str, Any]:
    return {
        "gallery": sorted(
            item["name"] for item in client.get("/static/tools/", headers=headers).json()["tools"]
        ),
        "categories": client.get("/static/categories/", headers=headers).json()["totalCount"],
    }


class TestDescribeRoutesIgnoreTheSessionFilter:
    def test_an_allowlist_header_does_not_narrow_the_catalog(self) -> None:
        """GIVEN a caller allowlisted to one tool, THEN both static routes still report both."""
    ...
    def test_an_unsupported_or_collapsing_mode_header_does_not_change_the_answer(
        ...
        with TestClient(_server(unfiltered=True).http_app()) as client:
            resp = client.get("/static/categories/", headers={"x-datarobot-mcp-mode": mode})
            gallery = client.get("/static/tools/", headers={"x-datarobot-mcp-mode": mode})
    ...
    def test_the_gallery_answers_json_when_the_catalog_cannot_be_built(self) -> None:
        ...
        with TestClient(_server(unfiltered=False).http_app()) as client:
            resp = client.get("/static/tools/", headers={"x-datarobot-mcp-mode": "code"})
