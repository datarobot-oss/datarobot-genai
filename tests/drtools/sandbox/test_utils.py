...
def dr_env() -> Iterator[None]:
    ...
    with (
        patch(
            "datarobot_genai.drtools.sandbox.workload.get_datarobot_access_token",
            return_value="test-token",
        ),
        patch("datarobot_genai.drtools.sandbox.workload.get_credentials") as mock_creds,
    ):
...
def _isolate_security_context_ff() -> Iterator[None]:
    ...
    with (
        patch(
            "datarobot_genai.drtools.sandbox.workload.request_user_dr_client",
            return_value=MagicMock(),
        ),
        patch(
            "datarobot_genai.drtools.sandbox.workload.FeatureFlag.is_enabled",
            return_value=False,
        ),
    ):
        yield
...
async def test_execute_code_happy_path(dr_env: None) -> None:
    ...
    with patch(
        "datarobot_genai.drtools.sandbox.workload.DataRobotWorkloadSandbox.run",
        new=mock_run,
    ):
...
async def test_execute_code_passes_security_context_when_ff_enabled(dr_env: None) -> None:
    from datarobot_genai.drtools.sandbox import workload as sandbox_workload

    mock_run = AsyncMock(return_value=_result())
    captured: dict[str, Any] = {}
    real_init = sandbox_workload.DataRobotWorkloadSandbox.__init__
    ...
    with (
        ...
        patch(
            "datarobot_genai.drtools.sandbox.workload.DataRobotWorkloadSandbox.__init__",
            new=_spy_init,
        ),
        patch(
            "datarobot_genai.drtools.sandbox.workload.FeatureFlag.is_enabled",
