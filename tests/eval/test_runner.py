...
def test_resolve_local_file_wins(tmp_path: Path) -> None:
    ...
    result = resolve_benchmark_module("evaluator/benchmarks/my_benchmark.py", tmp_path)
...
def test_resolve_falls_back_to_installed_package(tmp_path: Path) -> None:
    ...
    result = resolve_benchmark_module(
...
def test_resolve_strips_py_extension_for_import(tmp_path: Path) -> None:
    ...
    result = resolve_benchmark_module("datarobot_genai/eval/benchmarks/pii_leakage.py", tmp_path)
    assert Path(result).exists()
...
def test_resolve_raises_on_unresolvable_module(tmp_path: Path) -> None:
    ...
    with pytest.raises(ImportError):
        resolve_benchmark_module("totally/nonexistent/benchmark.py", tmp_path)
